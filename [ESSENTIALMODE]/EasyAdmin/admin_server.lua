	...
	RegisterCommand('crash', function(source, id, user)
		if source == 0 then
			if GetPlayerPing(id[1]) == 0 then
				...
				exports['exile_logs']:SendLog(source, "Użyto komendy /crash na graczu: ["..id[1].."]", "admin_commands")	
			end
		else
...
ESX.RegisterServerCallback('EasyAdmin:daneInnegoGracza', function(source, cb, target)
    ...
    if xPlayer ~= nil then
		local data = {
			...
			job = xPlayer.job,
			secondjob = xPlayer.secondjob,
			thirdjob = xPlayer.thirdjob,
...
ESX.RegisterCommand('admins', {'partner', 'patron', 'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'vip', 'user'}, function(xPlayerr, args, showError)
	local admins = {}
	for k, v in pairs(OnlineAdmins) do
		table.insert(admins, {label=GetPlayerName(k), value="admin"..k})
	end
	exports['exile_logs']:SendLog(xPlayerr.source, "Użyto komendy /admins", "admin_commands")
