ESX.RegisterCommand('goto', {'partner', 'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
    if args.id then
...
ESX.RegisterCommand('bring', {'partner', 'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
        if xPlayer and xTarget then
...
ESX.RegisterCommand('slap', {'best'}, function(xPlayer, args, showError)
    if args.id then
        ...
        if xPlayer and xTarget then
			...
			exports['exile_logs']:SendLog(xPlayer.source, "Użyto komendy /slap na graczu: ["..xTarget.source.."]", "admin_commands")
        end
    end
...
ESX.RegisterCommand('slay', {'best', 'partner', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
    if args.id then
        local xTarget = ESX.GetPlayerFromId(args.id)
        if xPlayer and xTarget then
			local xTarget = ESX.GetPlayerFromId(args.id)
            TriggerClientEvent('EasyAdmin:Slay', xTarget.source)
			exports['exile_logs']:SendLog(xPlayer.source, "Użyto komendy /slay na graczu: ["..xTarget.source.."]", "admin_commands")
        end
    end
...
ESX.RegisterCommand('tpp', {'partner', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('savecar', {'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.targetid ~= nil then
		local tPlayer = ESX.GetPlayerFromId(args.targetid)

		if tPlayer then
			TriggerClientEvent('EasyAdmin:GetCurrentCar', tPlayer.source, 2138)
		elseif xPlayer and tPlayer then
			TriggerClientEvent('EasyAdmin:GetCurrentCar', tPlayer.source, 2138)
...
ESX.RegisterCommand('updateplayer', {'mod', 'starszymod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('updateplate', {'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('givecar', {'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
    if args.steamid and args.targetid then
		local xPlayer = ESX.GetPlayerFromId(args.steamid)
		if xPlayer then
...
ESX.RegisterCommand('cleareq', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('delchar', {'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
		...
		MySQL.query('SELECT digit FROM characters WHERE identifier = @identifier', {
			...
			if result[1] then
				if tonumber(result[1].digit) == tonumber(args.digitid) then
					...
					MySQL.query('SELECT digit, accounts, inventory, skin, job, job_level, job_grade, job_id, secondjob, secondjob_grade, thirdjob, thirdjob_grade, position, firstname, lastname, dateofbirth, sex, height, status, isDead, phone_number, tattoos FROM characters WHERE identifier = @identifier', {
						['@identifier'] = args.steamid
					}, function(swap)
						if swap[1] then
							MySQL.update("UPDATE users SET digit = @digit, accounts = @accounts, inventory = @inventory, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, position = @position, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, tattoos = @tattoos WHERE identifier = @identifier", {
								['@identifier'] = args.steamid,
								['@digit'] = swap[1].digit,
								...
								['@secondjob'] = swap[1].secondjob,
								['@secondjob_grade'] = swap[1].secondjob_grade,
								['@thirdjob'] = swap[1].thirdjob,
								['@thirdjob_grade'] = swap[1].thirdjob_grade,
...
AddEventHandler('EasyAdmin:SaveCar', function(vehicleProps, recieved, xPgroup)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if not vehicleProps then return end
		if recieved == 2138 then
			MySQL.update('INSERT INTO owned_vehicles (owner, plate, vehicle, digit) VALUES (@owner, @plate, @vehicle, @digit)',
			{
			...
			exports['exile_logs']:SendLog(_source, "Próba wywołania eventu SaveCar", "anticheat")
		end


end)

ESX.RegisterCommand('przywrocpostac', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('revivedist', {'partner', 'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
    if args.dist then
		if args.dist <= 500 then
			...
			exports['exile_logs']:SendLog(xPlayer.source, "Użyto komendy /revivedist " .. tonumber(args.dist), "admin_commands")
		else
...
ESX.RegisterCommand('bitkiaddorg', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
...
ESX.RegisterCommand('bitkiaddgang', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM bitkigang WHERE org_name = @org_name', {
			...
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' winy dla gangu '..args.job, 'bitkimanage', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE bitkigang SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses + args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' lose dla gangu '..args.job, 'bitkimanage', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('bitkiremoveorg', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM bitki WHERE org_name = @org_name', {
			['@org_name'] = args.job
		}, function(result)
...
ESX.RegisterCommand('bitkiremovegang', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM bitkigang WHERE org_name = @org_name', {
			['@org_name'] = args.job
		}, function(result)
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' winow dla gangu '..args.job, 'bitkimanage', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE bitkigang SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses - args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' lose dla gangu '..args.job, 'bitkimanage', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('updateorgname', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.label ~= nil and args.number ~= nil then
		...
		MySQL.update('UPDATE strefy SET org_label = @org_label WHERE org_name = @org_name',{ 
			['@org_label'] = '#'..args.number..' '..args.label,
			['@org_name'] = args.job
		})
		MySQL.update('UPDATE napady SET org_label = @org_label WHERE org_name = @org_name',{ 
			['@org_label'] = '#'..args.number..' '..args.label,
			['@org_name'] = args.job
		})
...
ESX.RegisterCommand('spawn', {'partner', 'trialsupport', 'support', 'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
    if args.targetid then
        ...
        if xPlayerTarget then
			...
			if xPlayer then
				...
				exports['exile_logs']:SendLog(xPlayer.source, "Użył komendy /spawn na graczu: ["..args.targetid.."]", 'admin_commands', '5793266')
			end
        end
    end
...
ESX.RegisterCommand('strefyaddorg', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM strefy WHERE org_name = @org_name', {
			...
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' wygrane strefy dla organizacji '..args.job, 'admin_strefy', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE strefy SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses + args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' przegrane strefy dla organizacji '..args.job, 'admin_strefy', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('strefyremoveorg', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM strefy WHERE org_name = @org_name', {
			['@org_name'] = args.job
		}, function(result)
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' wygrane strefy dla organizacji '..args.job, 'admin_strefy', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE strefy SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses - args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' przegrane strefy dla organizacji '..args.job, 'admin_strefy', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('strefyaddgang', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM strefy_gangi WHERE org_name = @org_name', {
			...
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' wygrane strefy dla gangu '..args.job, 'admin_strefy', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE strefy_gangi SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses + args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' dodal '..args.wartosc..' przegrane strefy dla gangu '..args.job, 'admin_strefy', '5793266')
				end
			end
		end)
	end
...
ESX.RegisterCommand('strefyremovegang', {'mod', 'starszymod', 'admin', 'starszyadmin', 'superadmin', 'best', 'dev'}, function(xPlayer, args, showError)
	if args.job ~= nil and args.wartosc ~= nil and args.wartosc ~= nil then
		MySQL.query('SELECT wins,loses FROM strefy_gangi WHERE org_name = @org_name', {
			['@org_name'] = args.job
		}, function(result)
			if result[1] ~= nil then
				if args.what == 'win' then
					...
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' wygrane strefy dla gangu '..args.job, 'admin_strefy', '5793266')
				elseif args.what == 'lose' then
					MySQL.update('UPDATE strefy_gangi SET loses = @loses WHERE org_name = @org_name',{ 
						['@loses'] = result[1].loses - args.wartosc,
						['@org_name'] = args.job
					})
					exports['exile_logs']:SendLog(source, xPlayer.name..' usunal '..args.wartosc..' przegrane strefy dla gangu '..args.job, 'admin_strefy', '5793266')
				end
			end
		end)
	end
