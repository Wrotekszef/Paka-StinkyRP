function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, job, name, coords, character, secondjob, thirdjob, dealerLevel, digit, slots)
	local self = {}
		...
		if item then
			...
			if item.type == 'sim' then
				if self.character.phone_number == item.data.number then
					self.triggerEvent('e-phone:myPhoneNumber', nil)
