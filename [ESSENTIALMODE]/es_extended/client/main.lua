...
RegisterCommand('+-slot1', function()	
	...
	if exports['gcphone']:getMenuIsOpen() then return end
	Select(1, ped)
end)
...
function Select(number, ped)
	if not exports['esx_policejob']:IsCuffed() and not IsPedInAnyPoliceVehicle(ped) then
		local slot = ESX.GetfalszywyySlots()
