...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			radioPressed = true
			playMicClicks(true)
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(playerPed, false)) then
				if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "sheriff" or ESX.PlayerData.job.name == "ambulance" or ESX.PlayerData.job.name == "mechanik2" or ESX.PlayerData.job.name == "gheneraugarage" then
				...
				TriggerServerEvent("exilerp_scripts:addTalking")
			end
			CreateThread(function()
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "sheriff" or ESX.PlayerData.job.name == "ambulance" or ESX.PlayerData.job.name == "mechanik2" then
				ClearPedTasks(playerPed)
				TriggerServerEvent("exilerp_scripts:walkieTalkie", false)
			else
				StopAnimTask(playerPed, "random@arrests", "generic_radio_enter", -4.0)
			end	
			TriggerServerEvent("exilerp_scripts:stopTalking")
		end
