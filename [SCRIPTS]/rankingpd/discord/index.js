...
const DC_LEADERBOARD_CHANNEL_ID = GetConvar(
  'frakcje_dc_leaderboard_channel_id',
  'null'
);
...
if (DC_TOKEN !== 'null' && DC_APP_ID !== 'null') {
  ...
  client.on('ready', () => {
    ...
    setInterval(refreshrankingpd, 60 * 10000 * 1);
  });

  /* Leaderboard */
  const refreshrankingpd = () => {
    exports.oxmysql['query']('SELECT chlop, ile FROM rankingpd ORDER BY ile DESC', {},
    function(result) {
        if (result) {
          const embed = new RichEmbed()
            .setAuthor('ExileRP - Ranking System')
            .setTitle('Ranking TOP (MANDATY)')
          ...
          for (const index in result) {
            if (index < 10 && (result[index].wins != 0 || result[index].loses != 0)) {
              players.push(
                `** ${players.length+1}.** ${result[index].chlop} - Mandaty:** ${
                  result[index].ile
                }**`
              );
            }
