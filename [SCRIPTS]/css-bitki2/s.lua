...
RegisterCommand("bitkiorg2", function(src, args, raw) 
  if src > 0 then
    if tonumber(args[1]) then
      local xPlayer = ESX.GetPlayerFromId(src)
      local tPlayer = ESX.GetPlayerFromId(tonumber(args[1]))
      exports['exile_logs']:SendLog(src, "Wpisał /bitkiorg2 (Organizacja Przestępcza) "..xPlayer.thirdjob.name, 'bitkilogs', '3066993')
      local xOrg = nil
      local tOrg = nil
             ...
             if tPlayer.thirdjob.grade >= 2 then
              if string.sub(tPlayer.thirdjob.name, 1, 3) == "org" or string.sub(tPlayer.thirdjob.label, 1, 3) == "org" then
               if string.sub(tPlayer.thirdjob.name, 1, 3) == "org" then tOrg = tPlayer.thirdjob.label else tOrg = tPlayer.thirdjob.label end
                 TriggerClientEvent("bitkiorg2C:acceptLose", tPlayer.source, xOrg, xPlayer.thirdjob.name, xPlayer.name, tPlayer.name, src)
          else
...
RegisterNetEvent("bitkiorg2S:winResult")
AddEventHandler("bitkiorg2S:winResult", function(org, orgName, sender, recipient, screenshot, result, targetId) 
  ...
  if result == "accept" then
    ...
    MySQL.query("SELECT * FROM bitki2 WHERE org_name = @org_name", {
      ['@org_name'] = orgName,
    }, function (result)
      local wins = tonumber(result[1].wins)
      local loses = tonumber(result[1].loses)
      MySQL.update("UPDATE bitki2 SET wins = @wins WHERE org_name = @org_name", { 
        ['@org_name'] = orgName,
      ...
      MySQL.query("SELECT * FROM bitki2 WHERE org_name = @org_name", {
        ['@org_name'] = xOrgName,
          ...
          MySQL.update.await("UPDATE bitki2 SET loses = @loses WHERE org_name = @org_name", { 
            ['@org_name'] = xOrgName,
...
sendToDiscord = function (color, message, img, footer)
  ...
  PerformHttpRequest('https://discord.com/api/webhooks/1056667933155266560/TXqILcQnY3_tM52wa-tnpjiMW-KtQRKtpSoP812ZxSyeVXDxlfhnNJ55nK-Rbn_5bhfj', function(err, text, headers) end, 'POST', json.encode({username = "ExileRP - Bitki", embeds = embed}), { ['Content-Type'] = 'application/json' })
end