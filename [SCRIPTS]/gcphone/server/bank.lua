RegisterServerEvent('exile:przelew', function(data)
    local src = source
    ...
    if xPlayer.source ~= 0 then
        ...
        if amount and id then
            ...
            if bankMoney >= amount then
                ...
                MySQL.query('SELECT accounts, identifier, digit, phone_number FROM users WHERE account_number = ?', {id}, function(result)
                    if result[1] ~= nil then
                        ...
                        --if false then
                            ...
                            local bankMoney1 = xPlayer.getAccount('bank').money
                            if bankMoney1 < amount then
                                return
                            end
                            xPlayer.removeAccountMoney('bank', tonumber(amount))
                            string = xPlayer.character.firstname .. ' ' .. xPlayer.character.lastname
                            MySQL.update("INSERT INTO `bank_transfers` (`from`, `from_label`, `to`, `to_digit`, `title`, `money`) VALUES (@fromI, @fromL, @receiver, @receiverD, @title, @moneyA)", {
                            ...
                            if tPlayer then
                                ...
                                MySQL.update('UPDATE users SET accounts = JSON_SET(accounts, "$.bank", @newBank) WHERE identifier = @identifier',
                                {
                                    ['@identifier'] = result[1].identifier,
                                    ['@newBank'] = currentBank + amount
                                })
                                MySQL.update('INSERT INTO phone_messages (`transmitter`, `receiver`,`message`, `isRead`,`owner`) VALUES (@transmitter, @receiver, @message, @isRead, @owner);',
                                {
                                    ['@transmitter'] = 'Bank',
