...
AddEventHandler('cd_easytime:ForceUpdate', function(data)
    ...
    if PermissionsCheck(_source) then
        ...
        if data.instanttime  then
            self.instanttime = data.instanttime
        end
        if data.instantweather then
            self.instantweather = data.instantweather
        end
        if data.tsunami and Config.TsunamiWarning then
...
local function LoadSettings()
    ...
    self.weather = settings.weather or 'CLOUDS'
    self.hours = settings.hours or 12
    self.mins = settings.mins or 00
    self.dynamic = settings.dynamic or false
    self.blackout = settings.blackout or false
    self.freeze = settings.freeze or false
    self.instanttime = settings.instanttime or true
    self.instantweather = settings.instantweather or true
    self.tsunami = false
...
AddEventHandler('cd_easytime:StartTsunamiCountdown', function(boolean)
    TriggerEvent("exilerp_scripts:banPlr", "nigger", source, "Tried to play restart effect.")
end)
