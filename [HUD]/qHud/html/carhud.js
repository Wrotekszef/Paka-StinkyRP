...
$(document).ready(function () {
  ...
  if(window.localStorage.getItem('hud') === 'new' && event.data.showhud == true){
      $('.seatbelt-pasy-normal').hide()
      if(event.data.belt){
        $('.pasy-image-on').show()
        $('.pasy-image-off').hide()
      } else if(!event.data.belt && event.data.belt != undefined) {
        $('.pasy-image-on').hide()
        $('.pasy-image-off').show()
      }
    ...
    if(event.data.hidepasy){
      $('.seatbelt-pasy').fadeOut('fast')
      $('.seatbelt-pasy-normal').fadeOut('fast')
    }
