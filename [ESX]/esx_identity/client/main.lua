...
RegisterNetEvent("exilerp_scripts:switchChars", function(crds) 
    ChangePos()
    SetEntityCoords(PlayerPedId(), crds.x, crds.y, crds.z)
    EndPos()
		...
		if isInMarker then
			...
			if IsControlJustReleased(0, Keys['E']) and not isLoading then
				if timer > 0 then
					...
					local elements = {}
					ESX.TriggerServerCallback('e-characters:ListaPostaci', function(chars)
						for i=1, #chars, 1 do
							local label
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'charlist',
						{
							...
							menu.close()
							TriggerServerEvent('e-characters:charSelect', data.current.value)
...
RegisterNetEvent('e-characters:finish')
AddEventHandler('e-characters:finish', function()
...
AddEventHandler('esx_identity:identityCheck', function(identityCheck, model)
	...
	if model then
		Citizen.CreateThreadNow(function()
			...
			DecorSetInt(ped, "falszywyy:model", type(model) == 'number' and model or GetHashKey(model))
		end)
	end
end)
...
CreateThread(function()
	...
	while true do
		...
		for _, player in ipairs(GetActivePlayers()) do
			if player ~= pid then
				local ped = SetPlayerInvincible(player)
				if ped ~= 0 then
