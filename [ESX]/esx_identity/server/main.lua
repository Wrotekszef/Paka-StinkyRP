function getIdentity(source, callback)
	...
	MySQL.query('SELECT identifier, firstname, lastname, dateofbirth, sex, height FROM `users` WHERE `identifier` = ?', { xPlayer.identifier}, function(result)
...
function getCharacters(source, callback)
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.query('SELECT * FROM `characters` WHERE `identifier` = ?', { xPlayer.identifier}, function(result)
...
function changeIdentity(identifier, data, source, callback)
	...
	local userResult = MySQL.query.await('SELECT * FROM users WHERE identifier = ?', {identifier})
end

function setIdentity(identifier, data, source, callback)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local licenseTime = os.time() + (7 * 3600)
	TriggerEvent('esx_license:addAdvancedLicense', xPlayer.source, "drive", 0, licenseTime)
	TriggerEvent('esx_license:addAdvancedLicense', xPlayer.source, "ems_insurance", 0, licenseTime)
	TriggerEvent('esx_license:addAdvancedLicense', xPlayer.source, "oc_insurance", 0, licenseTime)
	local userResult = MySQL.query.await('SELECT * FROM users WHERE identifier = ?', {identifier})
	MySQL.query('SELECT * FROM characters WHERE identifier = ?', {identifier}, function(result)
		...
		if #result == 1 then
			MySQL.update('UPDATE characters SET accounts = @accounts, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, inventory = @inventory, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos, position = @position WHERE identifier = @identifier and digit = @digit', {
				...
				['@secondjob']		= xPlayer.getSecondJob().name,
				['@secondjob_grade'] = xPlayer.getSecondJob().grade,
				['@thirdjob']		= xPlayer.getThirdJob().name,
				['@thirdjob_grade'] = xPlayer.getThirdJob().grade,
				...
				xPlayer.setThirdJob('unemployed', 0)
				xPlayer.setAccountMoney('money', 800)
				xPlayer.setAccountMoney('bank', 5000)
				xPlayer.setAccountMoney('black_money', 0)
				Wait(2000)
				
				MySQL.update('UPDATE `users` SET `digit` = @digit, `firstname` = @firstname, `lastname` = @lastname, `job` = @job, `job_grade` = @job_grade, `job_level` = @job_level, `job_id` = @job_id, `secondjob` = @secondjob, `secondjob_grade` = @secondjob_grade, `thirdjob` = @thirdjob, `thirdjob_grade` = @thirdjob_grade, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, `status` = @status, `isDead` = @isDead, `tattoos` = @tattoos, `phone_number` = @phone_number, `position` = @position, account_number = @account_number WHERE identifier = @identifier', {
					['@identifier']		= identifier,
					['@digit']			= digitNumber,
					...
					if callback then
						...
						TriggerClientEvent('e-characters:finish', _source)
			...
			MySQL.update('INSERT INTO characters (identifier, digit, firstname, lastname, dateofbirth, sex, height, account_number) VALUES (@identifier, @digit, @firstname, @lastname, @dateofbirth, @sex, @height, @account_number)', {
				['@identifier']		= identifier,
				['@digit']			= digitNumber,
				['@firstname']		= data.firstname,
				['@lastname']		= data.lastname,
		...
		elseif #result == 0 then
			MySQL.update('UPDATE `users` SET `digit` = @digit, `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height WHERE identifier = @identifier', {
				['@identifier']		= identifier,
				['@digit']			= digitNumber,
				['@firstname']		= data.firstname,
				['@lastname']		= data.lastname,
			...
			MySQL.update('INSERT INTO characters (identifier, digit, firstname, lastname, dateofbirth, sex, height) VALUES (@identifier, @digit, @firstname, @lastname, @dateofbirth, @sex, @height)', {
				['@identifier']		= identifier,
				['@digit']			= digitNumber,
				['@firstname']		= data.firstname,
				['@lastname']		= data.lastname,
...
RegisterCommand('register', function(source, args, rawcommand)
	if source == 0 then
		...
		if xPlayer.getGroup() == 'superadmin' or xPlayer.getGroup() == 'best' then
			TriggerClientEvent('esx_identity:showRegisterIdentity', args[1], true)
			exports['exile_logs']:SendLog(xPlayer.source, "Użyto komendy /register " ..args[1], "admin_commands")
		else
...
RegisterCommand('updatename', function(source, args, rawcommand)
	if source == 0 then
		MySQL.update('UPDATE users SET firstname = @firstname, lastname = @lastname WHERE identifier = @identifier AND digit = @digit',
		{
			['@firstname'] = args[3],
			['@lastname'] = args[4],
			['@identifier'] = args[1],
		...
		MySQL.update('UPDATE characters SET firstname = @firstname, lastname = @lastname WHERE identifier = @identifier AND digit = @digit',
		{
			['@firstname'] = args[3],
			['@lastname'] = args[4],
			['@identifier'] = args[1],
...
function getCharacters(source, callback)
	...
	local data = {}
	MySQL.query("SELECT firstname, lastname, digit FROM `characters` WHERE `identifier` = @identifier",
	{
		['@identifier'] = identifier
	}, function(result)
		for i=1, #result, 1 do
...
ESX.RegisterServerCallback('e-characters:ListaPostaci', function(source, cb)
...
RegisterServerEvent('e-characters:charSelect')
AddEventHandler('e-characters:charSelect', function(charnum)
	...
	MySQL.update('UPDATE characters SET accounts = @accounts, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, inventory = @inventory, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos, position = @position WHERE identifier = @identifier and digit = @digit', {
		...
		if rowsChanged then
			MySQL.query('SELECT * FROM characters WHERE identifier = @identifier and digit = @digit', {
				['@identifier'] = xPlayer.identifier,
				['@digit']		= charnum
			}, function(characterResult)
				if characterResult then
					TriggerClientEvent("exilerp_scripts:switchChars", _source, json.decode(characterResult[1].position))
					...
					xPlayer.setSecondJob(characterResult[1].secondjob, characterResult[1].secondjob_grade)
					xPlayer.setThirdJob(characterResult[1].thirdjob, characterResult[1].thirdjob_grade)
					...
					MySQL.update('UPDATE `users` SET `digit` = @digit, accounts = @accounts, job_level = @job_level, job_id = @job_id, skin = @skin, `firstname` = @firstname, `lastname` = @lastname, `dateofbirth` = @dateofbirth, `sex` = @sex, `height` = @height, status = @status, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos WHERE identifier = @identifier', {
						...
						if rowsChanged2 then
							...
							TriggerClientEvent('e-phone:myPhoneNumber', _source, characterResult[1].phone_number)
							TriggerClientEvent('esx_property:reloadProperties', _source)
							TriggerClientEvent('e-characters:finish', _source)
							TriggerEvent("e-license:refresh", _source)
