...
function OpenShopMenu()
	...
	for k,v in ipairs(Categories) do
		...
		for j,d in ipairs(categoryVehicles) do
			...
			table.insert(options, d.name..(d.blocked == 1 and '<span style="color: red;"> Tylko podgląd</span>' or (ESX.PlayerData.job.name == 'cardealer' and '<span style="color: #7cfc00;"> $'..math.floor(d.price * 92 / 100)..'</span>' or '<span style="color: #7cfc00;"> $'..d.price..'</span>')))
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop',
	{
		...
		if (vehicleData.blocked == 0) then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
				title = vehicleData.blocked == 0 and 'Czy chcesz kupić '.. vehicleData.name .. ' za '.. ESX.Math.GroupDigits(vehicleData.price)..'$',
				align = 'center',
				...
				elseif data2.current.value == 'yes' then
					ESX.TriggerServerCallback("esx_scoreboard:getConnectedCops", function(ExilePlayers)
						if ExilePlayers then
							if ESX.PlayerData.job.name == 'cardealer' or ExilePlayers['cardealer'] == 0 then
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
											...
											TriggerServerEvent('exile_vehicles:setOwned', vehicleProps, vehicleData.name, ESX.PlayerData.job.name == 'cardealer' and ESX.Math.GroupDigits(math.floor(vehicleData.price * 93 / 100)) or ESX.Math.GroupDigits(vehicleData.price), 'car')		
										end)
...
AddEventHandler('esx_vehicleshop:hasEnteredMarker', function (zone)
	...
	elseif zone == 'ResellVehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) and ESX.PlayerData.job.name == 'cardealer' and ESX.PlayerData.job.grade >= Config.RequiredResselJobGrade then
