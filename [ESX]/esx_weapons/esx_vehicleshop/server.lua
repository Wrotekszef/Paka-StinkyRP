...
MySQL.ready(function()
	...
	local vehicles = MySQL.query.await('SELECT * FROM vehicles ORDER BY price ASC')
	BCategories    = MySQL.query.await('SELECT * FROM boats_categories')
	local boats    = MySQL.query.await('SELECT * FROM boats ORDER BY price ASC')
	PCategories    = MySQL.query.await('SELECT * FROM planes_categories')
	local planes   = MySQL.query.await('SELECT * FROM planes ORDER BY price ASC')
...
AddEventHandler('exile_vehicles:setOwned', function (vehicleProps, auto, cena, vehicleType)
	...
	MySQL.update('INSERT INTO owned_vehicles (owner, plate, vehicle, digit, type) VALUES (@owner, @plate, @vehicle, @digit, @type)',
	{
	...
	exports['esx_society']:SendLog("cardealer", "ExileRP", GetPlayerName(source).." Zakupiono pojazd: \nModel: " .. auto .. "\nNr. rej.: " .. vehicleProps.plate .. "\nCena: " .. cena .. "$", '3066993')
...
ESX.RegisterServerCallback('esx_vehicleshop:getCommercialVehicles', function (source, cb)
	MySQL.query('SELECT vehicle, price FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function (result)
		local vehicles = {}
				...
				if result[1] then -- does the owner match?
					...
					if vehicle.model == model then
						if vehicle.plate == plate then
							...
							cb(true)
							exports['esx_society']:SendLog("cardealer", 'ExileRP', GetPlayerName(source).." Sprzedał: " ..auto.. "\nRejestracja: "..plate.." \nCena: "..resellPrice, '5793266')
