...
MySQL.ready(function()
		...
		if shared == 0 then
			...
			for j=1, #result2, 1 do
				if Items[result2[j].name] then
					...
					table.insert(invalid_items, result2[j].name)
				end
			end
	...
	local DB_invalid_items_query = "DELETE FROM addon_inventory_items WHERE "
	if #invalid_items > 0 then
		for j=1, #invalid_items, 1 do
			if #invalid_items <= j then
				DB_invalid_items_query = DB_invalid_items_query .. "name=\""..invalid_items[j].."\";"
			else
				DB_invalid_items_query = DB_invalid_items_query .. "name=\""..invalid_items[j].."\" OR "
			end
		end
		MySQL.query.await(DB_invalid_items_query)
	end
end)
