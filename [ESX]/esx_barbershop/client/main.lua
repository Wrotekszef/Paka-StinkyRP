	...
	TriggerEvent('esx_skin:openRestrictedMenu', function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				ESX.TriggerServerCallback('esx_barbershop:checkMoney', function(hasEnoughMoney, moneytype)
					if hasEnoughMoney then
						TriggerServerEvent('esx_barbershop:pay', moneytype)
						TriggerEvent('skinchanger:getSkin', function(skin)
...
CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if #(coords - vec3(v.Pos.x, v.Pos.y, v.Pos.z)) < 7 then
				sleep = false				
				ESX.DrawMarker(vec3(v.Pos.x, v.Pos.y, v.Pos.z+0.1))
				ESX.ShowFloatingHelpNotification('Przeglądaj katalog z fryzurami', vec3(v.Pos.x, v.Pos.y, v.Pos.z+0.9))
			end
		end
