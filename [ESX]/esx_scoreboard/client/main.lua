...
AddEventHandler('esx:playerLoaded', function(playerData)
	...
	ESX.TriggerServerCallback('esx_scoreboard:ZapodajTrollAll', function(xd)
		antytroll = xd
	end)
	...
	if Ped.Active and Ped.Exists then
		...
		if IsEntityVisible(Ped.Id) then
			if not IsAdmin then
				...
				if not IsPedInAnyVehicle(Ped.Id, false) and not exports["esx_ambulancejob"]:isDead() and not IsEntityDead(Ped.Id) and not IsPedFalling(Ped.Id) and not IsPedCuffed(Ped.Id) and not IsPedDiving(Ped.Id) and not IsPedInCover(Ped.Id, false) and not IsPedInParachuteFreeFall(Ped.Id) and GetPedParachuteState(Ped.Id) < 1 and not exports['stinky_trunk']:checkInTrunk() and not exports['esx_policejob']:IsCuffed() then
...
RegisterNetEvent('esx_scoreboard:UstawStream', function(player, text)
	local ajdi = player
	local info = text
...
RegisterNetEvent('esx_scoreboard:troll', function(player, text)
	antytroll[player] = text
end)
								...
								if can then	
									...
									if #(coords1 - coords2) < dystans then
										...
										if ukryteid[ajdi] == false or ukryteid[ajdi] == nil then
											DrawText3D(coords2.x, coords2.y, coords2.z + 1.2, GetPlayerServerId(player)..(antytroll[aji] and "~r~." or ""), (NetworkIsPlayerTalking(player) and {0, 0, 255} or {255, 255, 255}))
											
											if (streamujetags[ajdi] ~= nil and tostring(streamujetags[ajdi]) ~= '') then
												DrawText3D(coords2.x, coords2.y, coords2.z + 1.5, streamujetags[ajdi], {128, 0, 255})
											end
										end
									end
								end
...
function PlayerList()
	...
	if timer - Timer > 10000 then
		Timer, Id, ExilePlayers = timer, nil, nil, {}
		TriggerServerEvent('esx_scoreboard:players')
	end
	...
	if Id and ESX.PlayerData.job then
		SendNUIMessage({
			...
			jobs   = {ems = ExilePlayers['ambulance'], police = ExilePlayers['police'], sheriff = ExilePlayers['sheriff'], mechanik = ExilePlayers['mechanik'], gheneraugarage = ExilePlayers['gheneraugarage'], doj = ExilePlayers['doj'], cardealer = ExilePlayers['cardealer'], player_count = ExilePlayers['players']}
		})
