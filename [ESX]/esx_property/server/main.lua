...
function ChangeStatePropety(name, state)
	MySQL.update('UPDATE properties SET is_available = @state WHERE name = @name', {
...
function SetPropertyOwned(name, price, rented, owner)
	...
	MySQL.update('INSERT INTO owned_properties (name, price, rented, owner, digit) VALUES (@name, @price, @rented, @owner, @digit)',
	{
		['@name']   = name,
		['@price']  = price,
...
function RemoveOwnedProperty(name, owner, price)
	local xPlayer = ESX.GetPlayerFromIdentifier(owner)
	MySQL.update('DELETE FROM owned_properties WHERE name = @name AND owner = @owner AND digit = @digit', {
		['@name']  = name,
		['@owner'] = owner,
		['@digit'] = xPlayer.getDigit()
	}, function(rowsChanged)
...
AddEventHandler('esx_property:setSubowner', function(name, target)
	...
	if xPlayer.getMoney() >= 30000 then
		local test = MySQL.query.await('SELECT co_owner1, co_owner2 FROM owned_properties WHERE name = @name AND owner = @owner AND digit = @digit',
		{
			['@name'] = pName,
		...
		if test[1].co_owner1 ~= nil and test[1].co_owner2 ~= nil then
			...
			if test[1].co_owner1 == nil then
				MySQL.update('UPDATE owned_properties SET co_owner1=@co_owner1, co_digit1 = @co_digit1 WHERE name = @name AND owner = @owner AND digit = @digit', 
				{
					['@owner'] 		= xPlayer.identifier,
					['@digit']		= xPlayer.getDigit(),
			...
			elseif test[1].co_owner2 == nil then
				MySQL.update('UPDATE owned_properties SET co_owner2=@co_owner2, co_digit2 = @co_digit2 WHERE name = @name AND owner = @owner AND digit = @digit', 
				{
					['@owner'] 		= xPlayer.identifier,
					['@digit']		= xPlayer.getDigit(),
...
RegisterServerEvent('falszywyy_properties:deleteSubowners')
AddEventHandler('falszywyy_properties:deleteSubowners', function(name)
...
AddEventHandler('onResourceStart', function(resourceName)
	if resourceName == GetCurrentResourceName() then
		MySQL.query('SELECT * FROM properties', function(properties)
			...
			MySQL.query('SELECT name, owner FROM owned_properties', function(ownedProperties)
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'property' .. property.name, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					exports['exile_logs']:SendLog(_source, "Mieszkanie: "..property.name.."\nWyciągnięto przedmiot: " .. item .. " x" .. count, 'property_get', '2123412')
				end
			else
...
AddEventHandler('esx_property:putItem', function(owner, type, item, count, property)
	...
	local xPlayer      = ESX.GetPlayerFromId(_source)
	local xPlayerOwner = ESX.GetPlayerFromIdentifier(owner)
	if item == "exilechip" then
		xPlayer.showNotification("Nie możesz chować żetonów!")
		return
	end
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getSharedInventory', 'property'..property.name, function(inventory)
				...
				if not string.find(sourceItem.label, "Chest") then
					-- MySQL.query('SELECT property_lvl FROM owned_properties WHERE owner = ? OR co_owner1 = ? OR co_owner2 = ?', {xPlayer.identifier, xPlayer.identifier, xPlayer.identifier}, function(result)
					-- 	if result ~= nil then
					-- 		local lvl = result[1].property_lvl
					-- 		local levels = {5.6, 5.8, 6.0, 6.5, 7.0, 7.5, 8.9, 8.5, 9.0, 9.5}
							-- if sourceItem.limit ~= -1 and (inventoryItem.count + count) > (sourceItem.limit * levels[lvl]) then
...
ESX.RegisterServerCallback('esx_property:getAllOwnedProperties', function(source, cb)
	...
	MySQL.query('SELECT name FROM owned_properties', function(ownedProperties)
		local properties = {}
...
ESX.RegisterServerCallback('esx_property:getOwnedProperties', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer then
		MySQL.query('SELECT owner, digit, co_owner1, co_owner2, co_digit1, co_digit2, name FROM owned_properties WHERE (owner = @owner AND digit = @digit) OR (co_owner1 = @owner AND co_digit1 = @digit) OR (co_owner2 = @owner AND co_digit2 = @digit)', {
		['@owner'] = xPlayer.identifier,
		['@digit'] = xPlayer.getDigit()
...
ESX.RegisterServerCallback('falszywyy_properties:getOwnedProperties', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.query('SELECT name FROM owned_properties WHERE owner = @owner AND digit = @digit AND rented = 0', {
		['@owner'] = xPlayer.identifier,
		['@digit'] = xPlayer.getDigit()
	}, function(ownedProperties)
		local properties = {}
...
AddEventHandler('esx_property:changeOwner', function(owner, name, price)
	...
	if xPlayer.getMoney() > price  then
		if xPlayerOwner ~= nil then
			MySQL.update('UPDATE owned_properties SET owner=@owner1, digit = @digit1, co_owner1 = NULL, co_digit1 = NULL, co_owner2 = NULL, co_digit2 = NULL WHERE name = @name AND owner = @owner2 AND digit = @digit2', 
			{
...
RegisterServerEvent('esx_property:upgrade', function()
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	if xPlayer then
		MySQL.query('SELECT property_lvl FROM owned_properties WHERE owner = ? OR co_owner1 = ? OR co_owner2 = ?', {xPlayer.identifier, xPlayer.identifier, xPlayer.identifier}, function(result)
			if result ~= nil then
				...
				if lvl < 10 then
					if money >= cost then
						MySQL.update('UPDATE owned_properties SET property_lvl = ? WHERE owner = ? OR co_owner1 = ? OR co_owner2 = ?', {lvl+1, xPlayer.identifier, xPlayer.identifier, xPlayer.identifier})
						xPlayer.showNotification('~b~Ulepszono pomyślnie!')
						xPlayer.removeMoney(cost)
					else
						xPlayer.showNotification('~r~Nie posiadasz gotówki na ulepszenie poziomu!')
					end
				else
					xPlayer.showNotification('~b~Twój level wynosi: ~g~'..lvl.. '/10 ~b~osiągnąłeś maksymalny poziom!')
				end
			end
		end)	
	end
...
RegisterServerEvent("esx_property:checkUpgrade", function()
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	if xPlayer then
		MySQL.query('SELECT property_lvl FROM owned_properties WHERE owner = ? OR co_owner1 = ? OR co_owner2 = ?', {xPlayer.identifier, xPlayer.identifier, xPlayer.identifier}, function(result)
			if result ~= nil then
				local lvl = result[1].property_lvl
				xPlayer.showNotification('~b~Poziom wynosi:~g~ '..lvl.. '/10')
			end
		end)	
	end
end)