...
ESX.RegisterServerCallback('exilerp_scripts:getPrivatesKeys', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local hex = xPlayer.getIdentifier()
	local digit = xPlayer.getDigit()
	if xPlayer then
		local z = {}
		for k,v in ipairs(owners) do
			if v.owner.identifier == hex and v.owner.digit == digit then
				table.insert(z, v)
			end
		end
		cb(z)
