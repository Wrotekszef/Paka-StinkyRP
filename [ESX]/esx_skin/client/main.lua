			...
			function(data, menu)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'skin_confirm', {
					...
					if data2.current.value then
						...
						if tworzypostac then
							TriggerServerEvent("tmsn701:KreatorBucket", false)
						end
						DeleteSkinCam()
	...
	CreateThread(function()
		...
		if FirstSpawn then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				if skin == nil then
                    TriggerEvent('falszywyy:newplayer')
                    skinLoaded = true
                else
...
RegisterNetEvent('esx_skin:openSaveableMenuNowy')
AddEventHandler('esx_skin:openSaveableMenuNowy', function(submitCb, cancelCb)
	tworzypostac = true
	OpenSaveableMenu(submitCb, cancelCb, nil)
end)
