			...
			getPhoneNumber(tonumber(k), function (n)
				if n ~= nil then
					if alert.numero ~= nil then
						...
						TriggerEvent('e-phone:_internalAddMessagexdxdtesttesttest', 'Centrala', n, text, 0, function (smsMess)
							TriggerClientEvent("e-phone:receiveMessage", tonumber(k), smsMess)
						end)
					end
				end
			end)
...
RegisterServerEvent("exile:przyjetoWezwanie", function(id) 
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		local name = xPlayer.character.firstname..' '..xPlayer.character.lastname
		
		TriggerClientEvent("exile:notifAccepted", id, name .. " przyjął twoje wezwanie")
