...
Config.MarkerColor  = {r = 54, g=227,b=175}
Config.MarkerType   = 23
Config.BlipType = 473
Config.Zones = {}
Config.Vault = {
	{
		Marker  = vector3(188.02598571777, -936.88177490234, 30.691789627075-0.90),
		Blip	= true,
		Hex = {},
	},
}
...
local blips = {{title="Schowek", colour=5, id= Config.BlipType, x = 188.02598571777, y = -936.88177490234, z = 30.691789627075},}
...
Citizen.CreateThread(function()
  while true do
    ...
    for i=1, #Config.Vault, 1 do
      if(GetDistanceBetweenCoords(pCoords, Config.Vault[i].Marker, true) < Config.DrawDistance) then
        if Config.Vault[i].Hex[1] ~= nil then
              ...
              if(GetDistanceBetweenCoords(pCoords, Config.Vault[i].Marker, true) < Config.MarkerSize.x) then
                  isInMarker = true
                  currentZone = 'exileMagazine_open'
              end
            end
					end
        else
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('xdsrakawtf:hasEnteredMarker', currentZone)
      end
      if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('xdsrakawtf:hasExitedMarker', LastZone)
     end
  end
end)
...
AddEventHandler('xdsrakawtf:hasEnteredMarker', function (zone)
	if zone == 'exileMagazine_open' then
		CurrentAction = 'schowekmenu'
	end
end)

AddEventHandler('xdsrakawtf:hasExitedMarker', function (zone)
...
function OpenRoomInventoryMenu()
	ESX.TriggerServerCallback('exile_magazine:getPropertyInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				menu.close()
        chowanieanim()
				TriggerServerEvent('exile_magazine:getItem', data.current.type, data.current.value, data.current.index)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					...
					if quantity == nil then
						...
						menu.close()
            chowanieanim()
						TriggerServerEvent('exile_magazine:getItem', data.current.type, data.current.value, quantity)
						ESX.SetTimeout(300, function()
							OpenRoomInventoryMenu()
						end)
					end
...
function OpenPlayerInventoryMenu()
	ESX.TriggerServerCallback('exile_magazine:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			if data.current.type == 'item_weapon' then
				menu.close()
				TriggerServerEvent('exile_magazine:putItem', owner, data.current.type, data.current.value, data.current.ammo)
        chowanieanim()
				ESX.SetTimeout(300, function()
					OpenPlayerInventoryMenu()
				end)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					...
					if quantity == nil then
						...
						menu2.close()
           chowanieanim()
						TriggerServerEvent('exile_magazine:putItem', data.current.type, data.current.value, tonumber(data2.value))
						ESX.SetTimeout(300, function()
							OpenPlayerInventoryMenu()
						end)
					end
