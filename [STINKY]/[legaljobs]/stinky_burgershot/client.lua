...
local Blips = {
	...
	{title="#2 Zbieranie Skladników", colour=5, id=126, see = true, coords = vector3(-2959.37, 371.74, 14.77)},
	{title="#3 Przeróbka Skladników", colour=5, id=126, see = true, coords = vector3(-1193.15, -904.28, 13.99)}
}
...
TransferingSeeds = function()
	...
	if count >= 100 then
		...
		ESX.ShowNotification('~r~Potrzebujesz 100 składników aby rozpocząć przerabianie!')
	end
end
...
AddEventHandler('exile_legalburgershot:hasEnteredMarker', function(zone, number)
	...
	if not IsPedInAnyVehicle(playerPed, false) then
		if zone == 'CollectSeeds' then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby zebrać składniki'
			CurrentActionData	= {}
		end
		...
		if zone == 'TransferingSeeds' and not cooldown then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby przerobić składniki'
			CurrentActionData	= {}
		end
