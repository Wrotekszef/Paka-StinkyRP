	...
	SetTimeout(60000, function()
		if PlayersHarvesting[source] == true then
			...
			if item.limit ~= -1 and item.count >= item.limit then
                ...
                TriggerEvent("exile_legalkrawiec:stopPickup")
			else
        ...
        if xPlayer.secondjob.name == 'krawiec' then
            if xPlayer.getInventoryItem('ubrania_krawiec').count == 0 then
                if countBefore >= 100 then
                    local xItem = xPlayer.getInventoryItem('material_krawiec')
                    if xItem.count == 100 then
                        ...
                        exports['exile_logs']:SendLog(_source, 'KRAWIEC: Próba zbugowania przeróbki! - gracz próbował przerabiać z ' .. xItem.count .. ' materialami (100 wymagane)', 'anticheat', '15844367')
                    end
                else
...
Paycheck = function(secondjob, xPlayer)
	...
	if secondjob == 'krawiec' then
        ...
        xPlayer.addInventoryItem('kwiatki', 8)
        xPlayer.showNotification('Otrzymano ~o~8x Kwiatki')
        xPlayer.addMoney(total)
        xPlayer.showNotification('Otrzymano ~g~'..total..'$ ~s~za dostarczenie.')
        exports['exile_logs']:SendLog(xPlayer.source, "KRAWIEC: Zakończono kurs. Zarobek: " ..total.. "$", 'krawiec', '15844367')
