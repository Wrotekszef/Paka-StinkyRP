...
local Blips = {
	...
	{title="#3 Przeróbka Proszku", colour=0, id=112, see = true, coords = vector3(-620.2, 208.07, 74.21)}
}
...
TransferingSeeds = function()
	...
	if count >= 100 then
		...
		ESX.ShowNotification('~r~Potrzebujesz 100 proszku aby rozpocząć przerabianie!')
	end
end
...
AddEventHandler('exile_legalxgamer:hasEnteredMarker', function(zone, number)
	...
	if not IsPedInAnyVehicle(playerPed, false) then
		if zone == 'CollectSeeds' then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby zebrać proszek'
			CurrentActionData	= {}
		end
		...
		if zone == 'TransferingSeeds' and not cooldown then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby przerobić proszek'
			CurrentActionData	= {}
		end
