...
RegisterNetEvent("exile_legalpizzeria:getrequest")
TriggerServerEvent("exile_legalpizzeria:request")
AddEventHandler("exile_legalpizzeria:getrequest", function(a, b, c)
	_G.donttouch = a
	_G.exile_krawic = b
	_G.exile_krawi2 = c

	local donttouchme = _G.donttouch
	local exile_legalpizzeria = _G.exile_krawic
	local exile_legalpizzeria2 = _G.exile_krawi2
...
local Cfg = {
	...
	Clothes = {
		Male = {
			...
			['torso_1'] = 460, ['torso_2'] = 7,
			['decals_1'] = 160, ['decals_2'] = 0,
			['arms'] = 132,
			['pants_1'] = 0, ['pants_2'] = 6,
...
RefreshBlips = function()
	if ESX.PlayerData.secondjob ~= nil and ESX.PlayerData.secondjob.name == 'pizzeria' then
		for i=1, #Blips, 1 do
			if Blips[i].see then
	...
	while true do
		...
		if ESX.PlayerData.secondjob ~= nil and ESX.PlayerData.secondjob.name == 'pizzeria' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('exile_legalpizzeria:hasEnteredMarker', currentZone, zoneNumber)
			end
	
			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('exile_legalpizzeria:hasExitedMarker', lastZone)
			end
...
CreateThread(function()
	while true do
		Wait(10)
		if ESX.PlayerData.secondjob and ESX.PlayerData.secondjob.name == 'pizzeria' then
...
StartCollect = function()
	...
	if count > 0 then
		...
		TriggerServerEvent('exile_legalpizzeria:collect', 'ciasto')
...
TransferingSeeds = function()
	...
	if count >= 100 then
		ESX.ShowNotification('~y~Trwa przygotowywanie Capricciosy...')
		...
		TriggerServerEvent(exile_legalpizzeria2, count, donttouchme)
		FreezeEntityPosition(playerPed, false)
	...
	if IsPedInAnyVehicle(playerPed, false) then
		...
		if ESX.Game.IsSpawnPointClear(GetEntityCoords(playerPed), 7) then
			ESX.Game.SpawnVehicle('rumpo1', GetEntityCoords(playerPed), 227.04, function(vehicle)
				...
				TriggerServerEvent('exile_legalpizzeria:insertPlayer', GetVehicleNumberPlateText(vehicle))
			end)
			alreadyOut = true
		else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pizzeria_boss', {
		...
		if data.current.value == '1' then
			if ESX.PlayerData.secondjob.grade_label == "Manager" then
				TriggerEvent('esxexile_societyrpexileesocietybig:openSecondBossMenu', 'pizzeria', function(data, menu)
					menu.close()
...
RegisterNetEvent('exile_legalpizzeria:Cancel')
AddEventHandler('exile_legalpizzeria:Cancel', function()
	FreezeEntityPosition(playerPed, false)
	ClearPedTasks(playerPed)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
...
AddEventHandler('exile_legalpizzeria:hasEnteredMarker', function(zone, number)
	ESX.UI.Menu.CloseAll()
	...
	if not IsPedInAnyVehicle(playerPed, false) then
		if zone == 'CollectSeeds' then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby zebrać ciasto'
			CurrentActionData	= {}
		end
		...
		if zone == 'TransferingSeeds' and not cooldown then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby przerobić ciasto'
			CurrentActionData	= {}
		end
...
AddEventHandler('exile_legalpizzeria:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('exile_legalpizzeria:stopPickup', zone)
...
OpenMobilepizzeriaActionsMenu = function()
	...
	if IsVehicleModel(vehicle, GetHashKey("rumpo1")) then
		if IsPedInAnyVehicle(playerPed, false) and GetPedInVehicleSeat(vehicle, -1) == playerPed then
			ESX.TriggerServerCallback('exile_legalpizzeria:checkSiedzacy', function(siedzi)
				if siedzi then
