...
RegisterNetEvent("exile_legalrzeznik:getrequest")
TriggerServerEvent("exile_legalrzeznik:request")
AddEventHandler("exile_legalrzeznik:getrequest", function(a, b, c)
	_G.donttouch = a
	_G.exile_krawic = b
	_G.exile_krawi2 = c

	local donttouchme = _G.donttouch
	local exile_legalrzeznik = _G.exile_krawic
	local exile_legalrzeznik2 = _G.exile_krawi2
...
local Cfg = {
	CollectSeeds = {
		{
			coords = vector3(1684.29, 6434.08, 32.27-0.95),
		},
	},
	TransferingSeeds = {
		{
			coords = vector3(-83.9, 6234.41, 31.09-0.95),
		},
	},
	Garage = {
		{
			coords = vector3(-59.16, 6279.22, 31.32-0.95)
		},
		{
			coords = vector3(1694.44, 6428.42, 32.62-0.95)
		},
	},
	BossActions = {
		{
			coords = vector3(-68.67, 6253.83, 31.09-0.95)
		},
	},
	Cloakroom = {
		{
			coords = vector3(-73.94, 6251.77, 31.08-0.95)
		},
		{
			coords = vector3(1698.5, 6425.41, 32.76-0.95)
		},
	},
	Clothes = {
        Male = {
            ['tshirt_1'] = 209, ['tshirt_2'] = 0,
            ['torso_1'] = 460, ['torso_2'] = 10,
        ...
        Female = {
            ...
            ['torso_1'] = 468, ['torso_2'] = 9,
            ['decals_1'] = 0, ['decals_2'] = 0,
            ['arms'] = 14,
            ['pants_1'] = 116, ['pants_2'] = 0,
            ['shoes_1'] = 154, ['shoes_2'] = 0,
            ['helmet_1'] = -1, ['helmet_2'] = 0,
            ['chain_1'] = 0, ['chain_2'] = 0,
            ['ears_1'] = -1, ['ears_2'] = 0,
...
local Blips = {
	{title="#1 Szatnia", colour=21, id=480, see = true, coords = vector3(-73.94, 6251.77, 31.08)},
	{title="#2 Zbieranie Skladników", colour=21, id=480, see = true, coords = vector3(1694.44, 6428.42, 32.62)},
	{title="#3 Przeróbka Skladników", colour=21, id=480, see = true, coords = vector3(-83.9, 6234.41, 31.09)}
}
...
RefreshBlips = function()
	if ESX.PlayerData.secondjob ~= nil and ESX.PlayerData.secondjob.name == 'slaughter' then
		for i=1, #Blips, 1 do
			if Blips[i].see then
	...
	while true do
		...
		if ESX.PlayerData.secondjob ~= nil and ESX.PlayerData.secondjob.name == 'slaughter' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('exile_legalrzeznik:hasEnteredMarker', currentZone, zoneNumber)
			end
	
			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('exile_legalrzeznik:hasExitedMarker', lastZone)
			end
...
CreateThread(function()
	while true do
		Wait(10)
		if ESX.PlayerData.secondjob and ESX.PlayerData.secondjob.name == 'slaughter' then
...
StartCollect = function()
	...
	if count > 0 then
		...
		TriggerServerEvent('exile_legalrzeznik:collect', 'slaughtered_chicken')
...
TransferingSeeds = function()
	...
	if count >= 100 then
		...
		TriggerServerEvent(exile_legalrzeznik2, count, donttouchme)
		FreezeEntityPosition(playerPed, false)
		ClearPedTasks(playerPed)
		TriggerEvent("exilerp_scripts:cooldown", -1)
	else
		ESX.ShowNotification('~r~Potrzebujesz 100 kurczaka aby rozpocząć przerabianie!')
	end
end
	...
	if IsPedInAnyVehicle(playerPed, false) then
		...
		if ESX.Game.IsSpawnPointClear(GetEntityCoords(playerPed), 7) then
			ESX.Game.SpawnVehicle('rumpo1', GetEntityCoords(playerPed), 227.04, function(vehicle)
				...
				TriggerServerEvent('exile_legalrzeznik:insertPlayer', GetVehicleNumberPlateText(vehicle))
			end)
			alreadyOut = true
		else
...
OpenBossMenu = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'slaughter_boss', {
		title    = "Praca Legalna",
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		if data.current.value == '1' then
			if ESX.PlayerData.secondjob.grade_label == "Manager" then
				TriggerEvent('esxexile_societyrpexileesocietybig:openSecondBossMenu', 'slaughter', function(data, menu)
					menu.close()
			...
			ESX.TriggerServerCallback('ExileRP:getCourses', function(kursy)
				if kursy then
					...
					for i=1, #kursy, 1 do
						...
						ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'slaughter_courses', elements, function(data, menu)
						end, function(data, menu)
							menu.close()
						end)
                    end
...
RegisterNetEvent('exile_legalrzeznik:Cancel')
AddEventHandler('exile_legalrzeznik:Cancel', function()
	FreezeEntityPosition(playerPed, false)
	ClearPedTasks(playerPed)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
...
AddEventHandler('exile_legalrzeznik:hasEnteredMarker', function(zone, number)
	ESX.UI.Menu.CloseAll()
	...
	if not IsPedInAnyVehicle(playerPed, false) then
		if zone == 'CollectSeeds' then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby zebrać zapakowanego kurczaka'
			CurrentActionData	= {}
		end
		...
		if zone == 'TransferingSeeds' and not cooldown then
			...
			CurrentActionMsg = 'Naciśnij ~INPUT_CONTEXT~, aby przerobić zapakowanego kurczaka'
			CurrentActionData	= {}
		end
...
AddEventHandler('exile_legalrzeznik:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('exile_legalrzeznik:stopPickup', zone)
	CurrentActionMsg = ''
	CurrentAction = nil
	blokada = false
end)

OpenMobileslaughterActionsMenu = function()
	...
	if IsVehicleModel(vehicle, GetHashKey("rumpo1")) then
		if IsPedInAnyVehicle(playerPed, false) and GetPedInVehicleSeat(vehicle, -1) == playerPed then
			ESX.TriggerServerCallback('exile_legalrzeznik:checkSiedzacy', function(siedzi)
				if siedzi then
