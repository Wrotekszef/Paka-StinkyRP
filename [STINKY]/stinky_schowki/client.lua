...
CreateThread(function()
	...
	branch = exports["es_extended"]:getServer()
	if string.find(branch, "dev") then
		print("Set branch to dev")
	end
...
function RefreshMagazines() 
	ESX.TriggerServerCallback("exilerp:fetchMagazines", function(magaziness) 
...
RegisterNetEvent("exilerp:updateMagazines", function() 
	RefreshMagazines()
end)
...
RegisterNetEvent("exilerp:openMenu", function(t)
	if t == "manage" then
		OpenManagment()
	end
end)

RegisterNetEvent("exilerp:setCurrentMagazine", function(mag) 
...
function OpenManagment() 
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "magazines_manage", {
				...
				function(data, menu)
					...
					if title ~= nil and tonumber(data.value) then
						if data.value <= 365 then
							TriggerServerEvent("exilerp:extendMagazine", data.value)
						else
		...
		elseif val == "upgrade_storage" then
			TriggerServerEvent("exilerp:magazineUpgrade", "storage")
		elseif val == "upgrade_coowner" then
			TriggerServerEvent("exilerp:magazineUpgrade", "coowner")
		elseif val == "add_coowner" then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), "coowner_add", {
				...
				if playerId ~= nil then
					TriggerServerEvent("exilerp:addCoOwner", playerId)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		elseif val == "remove_coowner" then
			ESX.TriggerServerCallback('exilerp:getCoowners', function(coowners)
				local elements2 = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), "coowner_remove", {
					...
					if playerIdentifier ~= nil then
						TriggerServerEvent("exilerp:removeCoOwner", playerIdentifier)
					end
				end, function(data2, menu2)
					menu2.close()
				end)
...
function OpenShared() 
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "shared_magazines", {
		...
		local val = data.current.value 
		menu.close()
		TriggerServerEvent("exilerp:enterMagazine", val)
	end, function(data, menu)
		menu.close()
	end)
end
...
function OpenMainMenu() 
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "magazines_interaction", {
		...
		if val == "enter_magazine" then
			TriggerServerEvent("exilerp:enterMagazine", magazines.owner[1].id)
		elseif val == "manage_magazine" then
			OpenManagment()
		elseif val == "buy_magazine" then
			TriggerServerEvent("exilerp:rentMagazine")
			...
			if dist1 < 12.0 then
				...
				if dist1 < 2.0 then
					...
					if IsControlJustPressed(0, 38) then
						TriggerServerEvent("exilerp:exitMagazine")
					end
				end
...
function OpenMagazineInv(owner, id) 
	if not exports['esx_policejob']:IsCuffed() then
		ESX.TriggerServerCallback('exilerp:getMagazineInventory', function(inventory)
		  ...
		  for weaponLabel, weaponData in pairs(weapons) do
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'magazine_inventory',
			{
				...
				if data.current.type == 'item_weapon' then
					...
					TriggerServerEvent('exilerp_magazine:getItem', owner, data.current.type, data.current.value, data.current.ammo, id)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('exilerp_magazine:getItem', owner, data.current.type, data.current.value, quantity, id)
							ESX.SetTimeout(300, function()
								OpenMagazineInv(owner, id)
							end)
						end
...
function OpenPlayerInv(owner,id) 
	...
	ESX.TriggerServerCallback('exilerp:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'putcount', {
				...
				if quantity == nil then
					...
					TriggerServerEvent('exilerp_magazine:putItem', owner, data.current.type, data.current.value, tonumber(data2.value), id)
					ESX.SetTimeout(300, function()
						OpenPlayerInv(owner,id)
					end)
				end
