...
RegisterServerEvent("exilerp:rentMagazine", function() 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        local money = xPlayer.getMoney()
        if money >= Config.Cost then
            MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit AND buyed = 1", {
                ...
                if res[1] then
                    ...
                    if money >= Config.Cost then
                        MySQL.Async.execute("DELETE FROM `exile_magazines` WHERE identifier = @identifier AND digit = @digit",{
                            ["@identifier"] = xPlayer.getIdentifier(),
                            ["@digit"] = xPlayer.getDigit()
                        })
                        ...
                        MySQL.Async.execute("INSERT INTO `exile_magazines` (`identifier`, `digit`, `coowner`, `coowner_digit`, `coowner1`, `coowner1_digit`, `coowner2`, `coowner2_digit`, `coowner3`, `coowner3_digit`, `coowner4`, `coowner4_digit`, `storage_level`, `coowner_level`, `buyed`, `expire`, `bucket`) VALUES (@identifier, @digit, '', '1', '', '1', '', '1', '', '1', '', '1', '1', '1', '1', @expire, @bucket)", {
                            ["@identifier"]=xPlayer.getIdentifier(),
                            ["@digit"]=xPlayer.getDigit(), 
                            ["@expire"]=expires,
                            ["@bucket"] = math.random(10000000,99999999)
                        }, function() 
                            TriggerClientEvent("exilerp:updateMagazines", xPlayer.source)
                        end)
...
RegisterServerEvent("exilerp:extendMagazine", function(days) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        ...
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit", {
            ["@identifier"] = xPlayer.getIdentifier(),
            ["@digit"] = xPlayer.getDigit()
        }, function(res) 
            if res[1] then
                ...
                local date = os.date("%d.%m.%Y %H:%M", expires)
                MySQL.Async.execute("UPDATE exile_magazines SET expire = @expires WHERE id = @id", {
                    ["@expires"] = expires,
                    ["@id"] = res[1].id
                }, function() 
                    TriggerClientEvent("exilerp:openMenu", xPlayer.source)
                    TriggerClientEvent("exilerp:updateMagazines", xPlayer.source)
                end)
                if not tonumber(res[1].buyed) then
                    MySQL.Async.execute("UPDATE exile_magazines SET buyed = 1 WHERE id = @id", {
                        ["@id"] = res[1].id
                    })
                end
...
ESX.RegisterServerCallback("exilerp:getCoowners", function(source, cb) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit", {
...
ESX.RegisterServerCallback('exilerp:getMagazineInventory', function(source, cb, owner, magazine)
    print(magazine)
	local xPlayer    = ESX.GetPlayerFromIdentifier(owner)
	local blackMoney = 0
	local items      = {}

	TriggerEvent('esx_addonaccount:getSharedAccount', 'magazine' .. magazine, function(account)
		blackMoney = account.money
	end)

	TriggerEvent('esx_addoninventory:getSharedInventory', 'magazine' .. magazine, function(inventory)
...
RegisterServerEvent('exilerp_magazine:getItem')
AddEventHandler('exilerp_magazine:getItem', function(owner, type, item, count, property)
	...
	if type == 'item_standard' then
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'magazine' .. property, function(inventory)
	...
	elseif type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getSharedAccount', 'magazine' .. property, function(account)
...
RegisterServerEvent('exilerp_magazine:putItem')
AddEventHandler('exilerp_magazine:putItem', function(owner, type, item, count, property)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
	local xPlayerOwner = ESX.GetPlayerFromIdentifier(owner)

    print(owner, type, item, count, property)
...
ESX.RegisterServerCallback('exilerp:getPlayerInventory', function(source, cb)
...
ESX.RegisterServerCallback("exilerp:fetchMagazines", function(source, cb) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        ...
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit", {
            ...
            MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE (coowner = @identifier AND coowner_digit = @digit) OR (coowner1 = @identifier AND coowner1_digit = @digit) OR (coowner2 = @identifier AND coowner2_digit = @digit) OR (coowner3 = @identifier AND coowner3_digit = @digit) OR (coowner4 = @identifier AND coowner4_digit = @digit)", {
                ["@identifier"] = xPlayer.getIdentifier(),
                ["@digit"] = xPlayer.getDigit()
            }, function(ress) 
                if ress[1] then
...
function setCoowner(id, which, identifier, digit,id1,id2) 
    MySQL.Async.execute("UPDATE exile_magazines SET "..which.." = @identifier, "..(which..'_digit').." = @digit WHERE id = @id", {
        ["@identifier"] = identifier,
        ['@digit'] = digit,
        ["@id"]=id
    })
    TriggerClientEvent("exilerp:updateMagazines", id1)
    TriggerClientEvent("exilerp:updateMagazines", id2)
    TriggerClientEvent("exilerp:openMenu", id1)
end

RegisterServerEvent("exilerp:addCoOwner", function(id) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent("exilerp:removeCoOwner", function(identifier) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    if xPlayer then
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit", {
            ...
            if res[1] then
                ...
                MySQL.Async.execute("UPDATE exile_magazines SET "..t.." = NULL WHERE id = @id", {
                    ["@id"] = res[1].id
                })
                TriggerClientEvent("exilerp:updateMagazines", xPlayer.source)
                TriggerClientEvent("exilerp:openMenu", xPlayer.source)
                if tPlayer then
                    TriggerClientEvent("exilerp:updateMagazines", tPlayer.source)
                end
...
RegisterServerEvent("exilerp:magazineUpgrade", function(what) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE identifier = @identifier AND digit = @digit", {
            ...
            if res[1] then
                ...
                if lvl == maxlvl then
                    ...
                    if xPlayer.getMoney() >= 150000 then
                        ...
                        newlvl = lvl+1
                        MySQL.Async.execute("UPDATE exile_magazines SET "..table.." = @newlvl WHERE id = @id", {
                            ["@newlvl"] = newlvl,
                            ["@id"] = res[1].id
                        })
                        TriggerClientEvent("exilerp:updateMagazines", xPlayer.source)
...
RegisterServerEvent("exilerp:enterMagazine", function(id) 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
        MySQL.Async.fetchAll("SELECT * FROM exile_magazines WHERE ((identifier = @identifier AND digit = @digit) OR (coowner = @identifier AND coowner_digit = @digit) OR (coowner1 = @identifier AND coowner1_digit = @digit) OR (coowner2 = @identifier AND coowner2_digit = @digit) OR (coowner3 = @identifier AND coowner3_digit = @digit) OR (coowner4 = @identifier AND coowner4_digit = @digit)) AND id = @id", {
            ["@identifier"] = xPlayer.getIdentifier(),
            ["@digit"] = xPlayer.getDigit(),
            ["@id"] = id
        }, function(ress) 
            if ress[1] then
                ...
                if os.time() >= ress[1].expire then
                    ...
                    if buyed then
                        MySQL.Async.execute("UPDATE exile_magazines SET buyed = 0 WHERE id = @id", {
                            ["@id"]=id
                        })
                    end
                    return
                end
                TriggerClientEvent("exilerp:setCurrentMagazine", xPlayer.source, ress[1])
...
RegisterServerEvent("exilerp:exitMagazine", function() 
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer then
