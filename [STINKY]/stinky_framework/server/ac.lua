...
RegisterServerEvent("gagri:gagri", function(source)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	if(xPlayer.identifier == 'discord:711168419252404264')then
		return
	else
		DropPlayer(src, "Weź offnij te x64a.rpf (zwiększone hitboxy) debilu no!")
	end
end)

local banning = {}
RegisterServerEvent("exilerp_scripts:banPlr", function(id, d, rsn, ac) 
    local src = source
    if tonumber(src) ~= nil then
      	TriggerEvent("exilerp_scripts:banPlr", "nigger", src, "Tried to ban player (Stinky Anticheat)")
    else
        src = d
        ...
        if tonumber(src) ~= nil or tonumber(src) ~= 0 then  
            ...
            local bannedby = "Exile Anticheat"
            ...
            MySQL.update("UPDATE exile_bans SET reason=@reason, expired=@expired, bannedby=@bannedby, isBanned=1 WHERE license=@license", {
                ...
                ["@discord"] = discord,
                ["@hwid"] = tokens,
                ["@reason"] = "[Exile Anticheat] You have been banned for cheating by Exile Anticheat. This ban never expires. If you think this is an error, please contact the server administrator.",
                ["@live"] = liveid,
                ["@xbl"] = xblid,
				...
				print("ban ", targetName, reason)
               	local ban_embed = {
					{
						["color"] = 15088186,
						["author"] = {
							["name"] = "Exile Anticheat",
							["url"] = "https://store.qfdevelopers.pl/",
							["icon_url"] = "https://cdn.discordapp.com/attachments/980183544565489725/1017504494411587666/store.qfdevelopers.pl.png"
						},
						...
						["thumbnail"] = {
							url = "https://cdn.discordapp.com/attachments/980183544565489725/1017504494411587666/store.qfdevelopers.pl.png"
						},
						["footer"] = {
							["text"] = "Exile Anticheat (c)"
						},
						["timestamp"] = os.date('!%Y-%m-%dT%H:%M:%S')
					}
				}
				-- PerformHttpRequest(Config.BanWebhook, function(err, text, headers) end, 'POST', json.encode({username = 'Exile Anticheat', avatar_url = 'https://cdn.discordapp.com/attachments/987789713102499923/988879893049786428/favicon.png', embeds = ban_embed}), { ['Content-Type'] = 'application/json' })
				if not ac then
					PerformHttpRequest("https://discord.com/api/webhooks/1052234175962427423/1-OvH7nuQqA-DtFnwLaVJ_FF_tSV62kI3MPwwc8NDz_5IdzA0dwMKTZcTjN7sffL7rCa", function(err, text, headers) end, 'POST', json.encode({username = 'Exile Anticheat', avatar_url = 'https://cdn.discordapp.com/attachments/987789713102499923/988879893049786428/favicon.png', embeds = ban_embed}), { ['Content-Type'] = 'application/json' })
                end
				banning[src] = false
            end)
            TriggerClientEvent("exilerp_scripts:crash", src)
			DropPlayer(src, "[Exile Anticheat] You have been banned for cheating by Exile Anticheat. This ban never expires. If you think this is an error, please contact the server administrator.")
        end
    end    
end)

RegisterServerEvent("exilerp_scripts:gotMeDMG", function(z) 
	local src = source
	TriggerEvent("exilerp_scripts:banPlr", "nigger", src, "Detection: "..z.." (Mods)")
end)

local payCheckStrikes = {}
local payCheckEvent = "exilerp_scripts:payCheck"
RegisterServerEvent(payCheckEvent, function(id) 
	local src = source
	if tonumber(src) ~= nil then
		TriggerEvent("exilerp_scripts:banPlr", "nigger", src, "Detection: Event "..payCheckEvent.." detected")
	else
		if payCheckStrikes[id] == nil then
			...
			if strikes > 5 then
				TriggerEvent("exilerp_scripts:banPlr", "nigger", id, "Detection: Paycheck spam detected, `"..strikes.."` in a row")
			else
...
local whTimeout = {}
RegisterNetEvent("exilerp_scripts:sendScreen", function(ss, key, count) 
	local src = source
	...
	print("^3[ExileRP] ^7Sending ^3INSERT^7 webhook for player ^3"..GetPlayerName(src).."^7")
	PerformHttpRequest(Config.InsertWebhook, function(err, text, headers) end, "POST", json.encode({username = "exilerp Inserts", embeds = embed}), { ["Content-Type"] = "application/json" })
	whTimeout[src] = true
...
AddEventHandler("exile:sendnibba", function(type, arg, skrypt)
	...
	if not doesPlayerHavePerms(_source, type) then
		TriggerEvent("exilerp_scripts:banPlr", "nigger", _source, string.format("Injection in script %s (Exile-Handler)", skrypt))
	end
end)
...
AddEventHandler("playerConnecting", function(playerName, setKickReason)
	for name in pairs(forbiddenNames) do
		if(string.gsub(string.gsub(string.gsub(string.gsub(playerName:lower(), "-", ""), ",", ""), "%.", ""), " ", ""):find(forbiddenNames[name])) then
			setKickReason("\n[exilerp AC] Twój nick jest niedozwolony, aby zagrać na serwerze zmień nick na poprawny!")
			CancelEvent()
			print("^3[ExileRP] "..playerName.."^7 was kicked for blacklisted name "..forbiddenNames[name])
			break
		end
	end
end)
	...
	if data.weaponType == 911657153 or data.weaponType == 1171102963 then
		...
		if tazedata[src] then
			if tazedata[src].count > 10 then
				...
				if isSpamTaze then
					CancelEvent()
					TriggerEvent(`exilerp_scripts:banPlr`, "nigger", src, "Detection: Tazed players "..tazedata[src].count.." times in 10 seconds")
				end
			end
		end
	end
