...
Rocade = function(steamID, discordID, licenseID, def, source)
	...
	if findChars == "BRAK POSTACI" then
		...
		def.presentCard(cardRegister, function(s, r)
			if s.x ~= nil then
				...
				exports['exile_logs']:SendLog(source, '**Stworzono nową postać!**\nImie: ' ..formattedFirstName.. '\nNazwisko: ' ..formattedLastName.. '\nPłeć: '..plec, 'characters', '5793266')
			else
	...
	repeat
		...
		Wait(Config.TimerRefreshClient * 1000)
...
RegisterIdentity = function(source, data)
	...
	for k, v in pairs(identifiers) do
		...
		if tonumber(data.height) > 200 then
			data.height = 200
		end
...
SetIdentity = function(source, charid, spawnPoint)
	...
	if license ~= nil then
		local time = os.time() + (7 * 3600)
		MySQL.update('INSERT INTO user_licenses (type, owner, digit, grade, time) VALUES (?, ?, ?, ?, ?)', {"drive", license, charid, 0, time}, function(rowsChanged)
		end)
		MySQL.update('INSERT INTO user_licenses (type, owner, digit, grade, time) VALUES (?, ?, ?, ?, ?)', {"ems_insurance", license, charid, 0, time}, function(rowsChanged)
		end)
		MySQL.update('INSERT INTO user_licenses (type, owner, digit, grade, time) VALUES (?, ?, ?, ?, ?)', {"oc_insurance", license, charid, 0, time}, function(rowsChanged)
		end)
		MySQL.query('SELECT * FROM users WHERE identifier = @identifier', {
			...
			if userResult[1].digit ~= charid then
				MySQL.update('UPDATE characters SET accounts = @accounts, skin = @skin, job = @job, job_grade = @job_grade, job_level = @job_level, job_id = @job_id, secondjob = @secondjob, secondjob_grade = @secondjob_grade, thirdjob = @thirdjob, thirdjob_grade = @thirdjob_grade, inventory = @inventory, firstname = @firstname, lastname = @lastname, dateofbirth = @dateofbirth, sex = @sex, height = @height, status = @status, isDead = @isDead, phone_number = @phone_number, account_number = @account_number, tattoos = @tattoos, position = @position WHERE identifier = @identifier and digit = @digit', {
					...
					MySQL.query('SELECT * FROM characters WHERE digit = @digit AND identifier = @identifier', {
						['identifier'] = license,
						['@digit'] = charid
				...
				if spawnPoint == 'airport' then
					MySQL.update('UPDATE users SET position = @position WHERE identifier = @identifier', {
						['@identifier'] = license,
...
ChooseCharacter = function(steamID, source)
	...
	MySQL.query('SELECT digit, firstname, lastname FROM characters WHERE identifier = @identifier', {
		['@identifier'] = license
	}, function(result)
		if result[1] then
			charLists[steamID] = {}
...
CheckBanlist = function(source)
	...
	if GetNumPlayerTokens(source) == nil or GetNumPlayerTokens(source) == 0 or GetNumPlayerTokens(source) < 0 or GetNumPlayerTokens(source) == "null" or GetNumPlayerTokens(source) == "**Invalid**" or not GetNumPlayerTokens(source) then
