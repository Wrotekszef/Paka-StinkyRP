...
RegisterNetEvent("exilerp_scripts:walkieTalkie")
AddEventHandler("exilerp_scripts:walkieTalkie", function(talking) 
	local src = source
	TriggerClientEvent("exilerp_scripts:setRadioIndicator", -1, src, talking)
end)
	...
	if item == "opaska" then
		if blips[playerId] then
			...
			if blips[playerId].opaska then
				...
				for k,v in pairs(blips) do
					if string.find(v.text, "[SASP]") or string.find(v.text, "[SASD]") then
