...
AddEventHandler("exile_gps:tempBlip", function(playerId, data) 
	...
	TriggerEvent("chatMessage", '^0[^3Centrala^0] ', { 255, 0, 0 }, "^*^1Opaska została zdjęta! Ostatnia lokalizacja została oznaczona na mapie")
	SetBlipSprite(blip, 1)
	SetBlipDisplay(blip, 4)
	SetBlipScale(blip, 0.85)
	SetBlipColour(blip, data.color)
...
RegisterNetEvent("exilerp_scripts:setRadioIndicator")
AddEventHandler("exilerp_scripts:setRadioIndicator", function(playerId, talking) 
	if blips[playerId] then
